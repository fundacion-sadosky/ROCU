...
bool hibachi_base::SerialPort::openConnection()
{
	...
	{
		// RCLCPP_ERROR_STREAM("Failed to connect to serial port '" << _serial.getPort() << "'");
		fprintf(stderr, "Failed to open serial port: %s\n", _serial.getPort().c_str());
        // return return_type::ERROR;
	}

	if (_serial.isOpen())
	{
		// RCLCPP_INFO_STREAM("Connected to serial port '" << _serial.getPort() << "'");
...
void hibachi_base::SerialPort::closeConnection()
{
	...
	{
		// RCLCPP_ERROR_STREAM("Failed to disconnect from serial port '" << _serial.getPort() << "'");
		fprintf(stderr, "Failed to disconnect from serial port: %s\n", _serial.getPort().c_str());
	}
}
...
hibachi_base::Message* hibachi_base::SerialPort::receiveMessage(uint16_t messageType)
{
	read();
...
hibachi_base::Message* hibachi_base::SerialPort::waitMessage(uint16_t messageType, double timeout)
{
	auto start_time = std::chrono::system_clock::now();
	while (true)
	{
...
void hibachi_base::SerialPort::read()
{
	...
	{
		// RCLCPP_ERROR_STREAM("Failed to receive data from serial port '" << _serial.getPort() << "'");
		fprintf(stderr, "Failed to receive data from serial port: %s", _serial.getPort().c_str());
		closeConnection();
	}
}
...
void hibachi_base::SerialPort::writeFrame(const uint8_t* data, size_t length)
{
	...
	{
		// RCLCPP_ERROR_STREAM("Failed to send data to serial port '" << _serial.getPort() << "'");
		fprintf(stderr, "Failed to send data to serial port: %s", _serial.getPort().c_str());
		closeConnection();
	}
}
